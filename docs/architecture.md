...
5. 按意图：
   ...
   - 待办/备忘/奶茶等办事：抽槽位 → 办事卡片（一次性 confirmToken）→ MCP 工具真调用（`ticket_create` / `ticket_query` / `memo_save` / `blog_*` / `tea_order`）
...
9. 控制台：总览 / 知识库 / 待办（状态机流转）/ 审核收件箱（HITL）/ 追踪回放 / 记忆快照 / MCP 工具与日志。
